...
import { FILE_DEFAULT_PATH, LOCALE } from "../api/config";
...
function FileIcon2({ entry }: { entry: FileEntry }) {
  if (entry.isDirectory)
    return (
      <Folder className="w-5 h-5 text-blue-400 fill-blue-400/20 shrink-0" />
    );
  const ext = getExt(entry.name);
  if (["json", "lock"].includes(ext))
    return <FileJson className="w-5 h-5 text-orange-400 shrink-0" />;
  if (["js", "ts", "jsx", "tsx", "py", "sh", "bat", "ps1"].includes(ext))
      ...
      {showNewDir && (
        <div className="flex items-center gap-2 p-3 bg-bg-card border border-blue-500/30 rounded-lg">
          ...
          <button
            ...
            className="px-3 py-1 text-text-secondary text-xs hover:text-white transition-colors"
          >
            Huỷ
          </button>
        </div>
          ...
          {isError && (
            <div className="flex items-center gap-2 p-4 text-red-400 text-sm">
              ...
              {(error as { response?: { data?: { message?: string } } })
                ...
                "Không thể tải thư mục — path không hợp lệ hoặc ngoài whitelist."}
            </div>
          )}
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                ...
                {!isLoading && currentPath !== DEFAULT_PATH && (
                  <tr
                    className="border-b border-border-subtle/30 hover:bg-white/5 cursor-pointer"
                    onClick={() => {
          ...
          <div className="bg-bg-card border border-border-subtle rounded-xl p-6 max-w-sm w-full mx-4 shadow-2xl">
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-4 py-2 text-sm text-text-secondary hover:text-white border border-border-subtle rounded-lg transition-colors"
              >
                Huỷ
              </button>
              <button
